...
const CALENDAR_IDS = [
  process.env.SUPERA_PARQUE,
  process.env.BIBLIOTECA_FRENTE,
  process.env.BIBLIOTECA_FUNDO,
  process.env.SALA_1_PREDIO_2,
  process.env.SALA_2_PREDIO_2,
  process.env.SALA_REUNIAO_PREDIO_1,
  process.env.SALA_TREINAMENTO,
  process.env.SALA_SUPERA_LAB,
];
...
export async function createGoogleCalendarEvent(eventData) {
  ...
  try {
    ...
    results.forEach((response, index) => {
      logger.info(
        `Eventos criados com sucesso - Link para agenda ${CALENDAR_IDS[index]}: ${response.data.htmlLink}`
      );
    });
    ...
    logger.error(
      "Erro ao criar evento numa das agendas do Google:",
      error.message
    );
    throw error;
  }
...
//   logger.info(
//     `Atualizando ${eventMappings.length} evento(s) para o estado cancelado...`
//   );
...
//       logger.info(
//         `- Atualizando evento ${mapping.eventId} na agenda ${mapping.calendarId}`
//       );
...
//     } catch (error) {
//       logger.error(
//         `Erro ao atualizar o evento ${mapping.eventId} na agenda ${mapping.calendarId}:`,
//         error.message
//       );
//       return null;
//     }
